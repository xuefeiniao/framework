...
class Request
{
    ...
    public function input($data = [], $name = '', $default = null, $filter = '')
    {
        ...
        $sqlKeywords = ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'FROM', 'WHERE', 'AND', 'OR', 'DROP', 'TABLE'];
        foreach ($data as  &$v) {
            //暂时有问题 注释
//            if ($host != "yufabu.saizhuge.net") {
//                //只有预发布才能跑sql
//                foreach ($sqlKeywords as $keyword) {
//                    if (strpos(strtoupper($v), $keyword) !== false) {
//                        $v = "";
//                    }
//                }
//            }
...
//            $v = htmlspecialchars($v);
            ...
            //双引号
            $count1 = substr_count($v, '"');
            $isEven1 = $count1 % 2 == 0;
            if (!$isEven1) {
                $v = str_replace('"', "", $v);
            }
            //`
            $count2 = substr_count($v, "`");
            $isEven2 = $count2 % 2 == 0;
            if (!$isEven2) {
                $v = str_replace('`', "", $v);
            }
