...
//                    $member_id = session("member_auth.member_id") ?: MemberID;
//                    $user_id = session("member_auth.uid") ?: MemberUID;
//                    $listRows = cache("list_rows_memberid_{$member_id}_memberuid_{$user_id}");
//                    if (!$listRows) {
//                        $listRows = $config["query"]["list_rows"] ?: $config['list_rows'];
//                    }
//                } else {
//                    $listRows = $config["query"]["list_rows"] ?: $config['list_rows'];
//                }
    ...
    public function delete($data = null)
    {
        ...
        if (!empty($whereAnd)) {
            $count = count($whereAnd);
            $whereStr = json_encode($whereAnd);
            if ($count == 1) {
                //只有is_del 不允许删除
                if (strpos($whereStr, 'is_del') !== false) {
                    throw new Exception('更新条件非法，不允许大批量更新数据');
                }
                //只有aid 大于什么 不许更新
                if (strpos($whereStr, '["aid",">"') !== false) {
                    throw new Exception('更新条件非法，不允许大批量更新数据');
                }
                //只有aid 小于什么 不许更新
                if (strpos($whereStr, '["aid","<"') !== false) {
                ...
                if (
                    strpos($whereStr, 'member_id') !== false
                    && strpos($whereStr, 'is_del') !== false
                ) {
                    throw new Exception('更新条件非法，不允许大批量更新数据');
                }
